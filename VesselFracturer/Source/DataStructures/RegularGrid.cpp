...
{
	ComputeShader* shader = ShaderList::getInstance()->getComputeShader(ShaderEnum::DETECT_BOUNDARIES);
...
{
	_assignVertexClusterShader = ShaderList::getInstance()->getComputeShader(ShaderEnum::ASSIGN_VERTEX_CLUSTER);
	_countQuadrantOccupancyShader = ShaderList::getInstance()->getComputeShader(ShaderEnum::COUNT_QUADRANT_OCCUPANCY);
	_countVoxelTriangleShader = ShaderList::getInstance()->getComputeShader(ShaderEnum::COUNT_VOXEL_TRIANGLE);
	_erodeShader = ShaderList::getInstance()->getComputeShader(ShaderEnum::ERODE_GRID);
	_pickVoxelTriangleShader = ShaderList::getInstance()->getComputeShader(ShaderEnum::SELECT_VOXEL_TRIANGLE);
	_resetCounterShader = ShaderList::getInstance()->getComputeShader(ShaderEnum::RESET_BUFFER);
	_undoMaskShader = ShaderList::getInstance()->getComputeShader(ShaderEnum::UNDO_MASK_SHADER);
}
